...
def create_labeled_video(config,videos,videotype='avi',shuffle=1,trainingsetindex=0,filtered=False,save_frames=False,Frames2plot=None,delete=False,displayedbodyparts='all',codec='mp4v',outputframerate=None, destfolder=None,draw_skeleton=False,trailpoints = 0,displaycropped=False):
    ...
    for video in Videos:
        ...
        print(f"Video type: {videotype}")
        vname = video.stem

        if filtered==True:
            videooutname = f"{vname}_{DLCscorer}_filtered_labeled.mp4"
        else:
            videooutname = f"{vname}_{DLCscorer}_labeled.mp4"

        if (videofolder / videooutname).is_file():
            print("Labeled video already created.")
        else:
            print("Loading ", video, "and data.")
            dataname = videofolder / f"{vname}_{DLCscorer}.h5"

            Dataframe = None
            try:
                Dataframe = pd.read_hdf(str(dataname))
                metadata=auxiliaryfunctions.LoadVideoMetadata(dataname)
                #print(metadata)
                datanames=[dataname]
            except FileNotFoundError:
                pass # Dataframe will be left None

            if Dataframe is None:
                datanames=[fn for fn in videofolder.iterdir() if (vname in fn.stem) and (fn.suffix == '.h5') and ("resnet" in fn.stem)]
                if len(datanames)==0:
                    ...
                    DLCscorer='DeepCut' + datanames[0].stem.split('DeepCut')[1]
            ...
            if len(datanames)>0:
                ...
                if cropping == True:
                    print(f"video will be cropped (x1={x1}, x2={x2}, y1={y1}, y2={y2})")
                else:
                    print("video will not be cropped")
